...
import { collectImages, resolveImage } from '../../src/render/images';
import { loadMediaIndex, uploadMedia } from '../../src/target/wordpress/media';
import { WpClient } from '../../src/target/wordpress/client';
import { contentHash } from '../../src/util/hash';
import { createFakeWp } from '../fixtures/fake-wp';

const ctx = (present: string[]) => ({
  sourceAbsolutePath: '/site/docs/guides/sync.md',
  contentPath: '/site/docs',
  contentPathLocalized: '/site/i18n/fr/docs',
  staticDirs: ['/site/static'],
  baseUrl: '/project/',
  siteDir: '/site',
...
test('a relative image resolves against the document', () => {
  const resolved = resolveImage('./img/a.png', ctx(['/site/docs/guides/img/a.png']));
  assert.deepEqual(resolved, {
    kind: 'file',
    url: './img/a.png',
    file: '/site/docs/guides/img/a.png',
  });
});

test('a localised copy of an asset is preferred when there is one', () => {
  const resolved = resolveImage('./img/a.png', ctx(['/site/i18n/fr/docs/guides/img/a.png']));
  assert.equal(resolved.kind, 'file');
  assert.equal((resolved as { file: string }).file, '/site/i18n/fr/docs/guides/img/a.png');
});

test('a root-relative image comes from a static directory, with or without the base URL', () => {
  assert.equal(resolveImage('/img/a.png', ctx(['/site/static/img/a.png'])).kind, 'file');
  assert.equal(resolveImage('/project/img/a.png', ctx(['/site/static/img/a.png'])).kind, 'file');
});

test('a site-relative image uses the @site prefix', () => {
  const resolved = resolveImage('@site/static/img/a.png', ctx(['/site/static/img/a.png']));
  assert.equal(resolved.kind, 'file');
});
